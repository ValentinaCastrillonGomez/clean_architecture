...
public class ReAssignUserTasksUseCaseTest {
    ...
    private final TaskToDo task1 = TaskToDo.builder().id("1").name("Task 1").description("Task Desc").reportStatus(TaskToDo.TaskReportStatus.ASSIGNED).assignedUserId("56").build();
    private final TaskToDo task2 = TaskToDo.builder().id("2").name("Task 2").description("Task Desc").reportStatus(TaskToDo.TaskReportStatus.ASSIGNED).assignedUserId("56").build();
    private final TaskToDo task3 = TaskToDo.builder().id("3").name("Task 3").description("Task Desc").reportStatus(TaskToDo.TaskReportStatus.ASSIGNED).assignedUserId("56").build();
        ...
        StepVerifier.create(savedFlux)
            .assertNext(task1 ->
                assertThat(task1).extracting(TaskToDo::getAssignedUserId, TaskToDo::getId, TaskToDo::getReportStatus)
                    .containsExactly(null, "1", TaskToDo.TaskReportStatus.PENDING_REASSIGNMENT)
            )
            .assertNext(task1 ->
                assertThat(task1).extracting(TaskToDo::getAssignedUserId, TaskToDo::getId, TaskToDo::getReportStatus)
                    .containsExactly(null, "2", TaskToDo.TaskReportStatus.PENDING_REASSIGNMENT)
            )
            .assertNext(task1 ->
                assertThat(task1).extracting(TaskToDo::getAssignedUserId, TaskToDo::getId, TaskToDo::getReportStatus)
                    .containsExactly(null, "3", TaskToDo.TaskReportStatus.PENDING_REASSIGNMENT)
            ).verifyComplete();
    }
}